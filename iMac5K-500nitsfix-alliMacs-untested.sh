...
# Compatible with iMac15,1, iMac17,1, and iMac18,3.
#
set -euo pipefail
...
# Locate the SSDT declaring Method (ABCL)
...
# 4. Decompilation & Dynamic AST Patching
...
assert n == 1, "DefinitionBlock revision could not be bumped"

# 2. Locate Method (ABCL)
idx = s.find("Method (ABCL,")
assert idx != -1, "Method (ABCL) not found in table"
...
# Check if already patched to 4..100 (Package of 99 items = 0x63)
if "Package (0x63)" in method_body:
    print("  [Python] Table already reflects 99 brightness entries (levels 4..100).")
    sys.exit(3)

# 3. Locate the actual levels Package inside Method (ABCL) dynamically
...
# Construct 4..100 levels (97 levels + 2 defaults = 99 items = 0x63)
levels = list(range(4, 101))
formatted = ['0x64', '0x32'] + [f'0x{lvl:02X}' for lvl in levels]
new_levels_str = ",\n                        ".join(formatted)

new_pkg = (
    pkg_match.group(1) + "0x63" + pkg_match.group(2) +
    "\n                        " + new_levels_str + "\n                    " +
    pkg_match.group(4)
)
...
print("  [Python] Replaced ABCL return package: injected levels 4..100 (99 values).")
